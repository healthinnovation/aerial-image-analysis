...
from lib.dataset import CGIARDataset
...
if __name__ == "__main__":
    ...
    trainparser = argparse.ArgumentParser(description='[CGIAR] Parameters for training', allow_abbrev=False)
    ...
    wandb.init(project="CGIAR", entity="gabrieljg")
    ...
    gt_class = conf.dataset.gt_class
    ...
    if test_file != 'none':
        test_dataset = CGIARDataset(meta_data=test_file,
                                    root_dir=data_dir,
                                    gt_class=gt_class,
                                    cache_data=cache_data,
                                    transform=transforms.Compose([
        ...
        test_dataloader = DataLoader(test_dataset, batch_size=1,
                            shuffle=False, num_workers=conf.train_par.workers, pin_memory=False)

    train_dataloader = DataLoader(train_dataset, batch_size=conf.train_par.batch_size,
                            shuffle=True, num_workers=conf.train_par.workers, pin_memory=False)
