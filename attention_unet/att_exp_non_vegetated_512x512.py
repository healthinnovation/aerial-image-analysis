...
def train_pytorch_model():
    ...
    full_img = Image.open(os.path.join('..', '..', 'dataset_512x512', '2018_11_13_1__pissy_mos', 'non_vegetated', 'patches', 'patch_0000.png'))
    ...
    full_mask = Image.open(os.path.join('..', '..', 'dataset_512x512', '2018_11_13_1__pissy_mos', 'non_vegetated', 'masks', 'mask_0000.png'))
    ...
    for epoch in range(n_epoch):
        ...
        for batch in tqdm(train_loader):
            ...
            with torch.no_grad():
                ...
                d4_out_pil = np.array(d4_out_pil.resize((512, 512)))
...
if __name__ == '__main__':
    ...
    batch_size = 4 # batch size for one GPU
    ...
    for csv in train_csvs:
        ...
        tmp_train_list = train_df.folder.to_list()
    ...
    for csv in val_csvs:
        ...
        tmp_val_list = val_df.folder.to_list()
        val_slides_folds.append(tmp_val_list)
 
    # Path to the reference image (for normaliztation)
    ref_image_path = os.path.join(main_data_dir, '2018_11_13_1__pissy_mos', 'non_vegetated', 'patches', 'patch_0000.png')

    # Online normalization flag
    normalize_switch = False

    if not(normalize_switch): offline_normalization = 'patches'
    else: 'patches'
    #######################################################################
    ...
    path_to_logs = os.path.join('..', '..', 'heat_experiments_non_vegetated', exp_name)
    ...
    for k in range(len(test_slides)):
        tmp_test_imgs = natsorted(glob(os.path.join(main_data_dir,test_slides[k], classCGIAR, offline_normalization,'*.png')))
        tmp_test_masks = natsorted(glob(os.path.join(main_data_dir,test_slides[k], classCGIAR, 'masks','*.png')))
    ...
    for i in range(cross_val):
        if not(folds_done[i]):
            ...
            for k in range(len(val_dir_fold)):
                tmp_val_imgs = natsorted(glob(os.path.join(main_data_dir,val_dir_fold[k], classCGIAR, offline_normalization,'*.png')))
                tmp_val_masks = natsorted(glob(os.path.join(main_data_dir,val_dir_fold[k], classCGIAR, 'masks','*.png')))
            ...
            for k in range(len(train_dir_fold)):
                tmp_train_imgs = natsorted(glob(os.path.join(main_data_dir,train_dir_fold[k], classCGIAR, offline_normalization,'*.png')))
                tmp_train_masks = natsorted(glob(os.path.join(main_data_dir,train_dir_fold[k], classCGIAR, 'masks','*.png')))
