...
def find_max(im_pths):
    ...
    for i in im_pths:
        #print(type(np.load(str(i))))
        #print(np.squeeze(np.load(str(i))).shape)
        img = np.squeeze(np.load(str(i)))
        #print(img.shape)   
...
def cal_dir_stat(im_pths, maximunValue,CHANNEL_NUM): ##give the names 
    ...
    for path in im_pths:
        #im = np.squeeze(np.load(path)).transpose(1,2,0) #  because input imgs is CHXHXW
        im = np.squeeze(np.load(path)) #  because input imgs is CHXHXW
        im= im[:,:,:CHANNEL_NUM]
        im = im/maximunValue
...
def meanstd(train_root,val_root, test_root,rootdata='dataset/data_512',CHANNEL_NUM='6'): #name_file,
    ...
    print('data_all_raiz:', data_path) # 0-1
    data_all_raiz=str(data_path/'images')  
    get_files_path = str(data_all_raiz) + "/*.npy"
    print('data_all_raiz:', data_all_raiz) # 0-1
    ...
    print("max train: ", maximo_pixel_train)  

  
    minimo_pixel_val,maximo_pixel_val,size_val = find_max(val_root)
    print("max val: ", maximo_pixel_val) 
    ...
    print("max val all: ", maximo_pixel_all)  



    print('Train:',str(data_path), size_train, 'min ', minimo_pixel_train,'max ',maximo_pixel_train) # 0-1
    print('Val:',str(data_path), size_val,'min ', minimo_pixel_val,'max ',maximo_pixel_val) # 0-1
    print('Test:',str(data_path), size_test,'-min ', minimo_pixel_test,'max ',maximo_pixel_test) # 0-1        
    print('All:',str(data_path), size_all,'min ', minimo_pixel_all,'max ',maximo_pixel_all) # 0-1
