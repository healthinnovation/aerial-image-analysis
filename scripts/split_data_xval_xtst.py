...
if __name__ == "__main__":
    '''
    [CGIAR] Script that divides data into N groups. Each group will form a fold for the 
    cross-validation (leave-one-fold-out) and cross-testing. To divide the data into N groups, 
    we first shuffle the data. 
    '''
    parser = argparse.ArgumentParser()

    parser.add_argument("--data_csv", type=str, help="This is the csv with folders containing a class.", required=True)
    ...
    parser.add_argument("--save_dir", type=str, help="Where the csv containing the division of the samples need to be saved", default='../data')

    args = parser.parse_args()

    gt = args.data_csv.split('.csv')[-2].split('/')[-1]
    drone_imgs = pd.read_csv(args.data_csv)
    drone_imgs = drone_imgs.drop('number',1)
    #drone_imgs = pd.DataFrame({'images': os.listdir(args.data_dir)})

    imgFolds = np.array_split(drone_imgs.sample(frac=1), args.num_folds) # Shuffle and split

    output_dir = os.path.join(args.save_dir, f'experiment_{gt}_{args.num_exp:03}')
    os.makedirs(output_dir, exist_ok=True)

    assert len(imgFolds) == args.num_folds, f'Consider a different number of folds so number of WSI is divisible.'
    ...
    for i in range(args.num_folds):
        ...
        imgFolds[i].to_csv(testfile, index=False)

        # Join the remaining folds and do cross validation
        df = pd.concat([x for j, x in enumerate(imgFolds) if j != i])
